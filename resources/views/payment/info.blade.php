        ...
        <div class="card-body">
            <div class="row">
                <div class="col-12">
                    ...
                    <form action="{{ route('student_register_show') }}" method="GET"><button type="submit" class="float-right border border-0 button_register"><span class="float-right span_register">Yeni Öğrenci Kayıt</span><i class="fas fa-plus-circle float-right icon_register pr-2"></i></button></form>
                </div>
            </div>
            <div class="row mt-3">
                    ...
                    <table id="payment-table" class="payment-table table table-striped">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="align-middle">{{ number_format($payment->debt_amount,2,",",".") }}@if($payment->currency_unit=="Türk Lirası") TL @elseif($payment->currency_unit=="Dolar") $ @endif</td>
                                ...
                                <td class="align-middle">{{ number_format($payment->total_remaining_amount,2,",",".") }}@if($payment->currency_unit=="Türk Lirası") TL @elseif($payment->currency_unit=="Dolar") $ @endif</td>
                                <td class="align-middle">@if(isset($payment->person->agency_id)){{ $payment->person->agency->name }}<br>{{ number_format($payment->agency_debt_amount-$payment->agency_paid_amount,2,",",".")}}@if($payment->currency_unit=="Türk Lirası") TL @elseif($payment->currency_unit=="Dolar") $ @endif @endif</td>
                                <td class="align-middle">@if(isset($payment->installment1_remaining_amount)){{ number_format($payment->installment1_remaining_amount,2,",",".") }}@if($payment->currency_unit=="Türk Lirası") TL @elseif($payment->currency_unit=="Dolar") $ @endif <br> {{ $payment->installment_date_format(1) }}@endif</td>
                                <td class="align-middle">@if(isset($payment->installment2_remaining_amount)){{ number_format($payment->installment2_remaining_amount,2,",",".") }}@if($payment->currency_unit=="Türk Lirası") TL @elseif($payment->currency_unit=="Dolar") $ @endif <br> {{ $payment->installment_date_format(2) }}@endif</td>
                                <td class="align-middle">@if(isset($payment->installment3_remaining_amount)){{ number_format($payment->installment3_remaining_amount,2,",",".") }}@if($payment->currency_unit=="Türk Lirası") TL @elseif($payment->currency_unit=="Dolar") $ @endif <br> {{ $payment->installment_date_format(3) }}@endif</td>
                                <td class="align-middle">@if(isset($payment->installment4_remaining_amount)){{ number_format($payment->installment4_remaining_amount,2,",",".") }}@if($payment->currency_unit=="Türk Lirası") TL @elseif($payment->currency_unit=="Dolar") $ @endif <br> {{ $payment->installment_date_format(4) }}@endif</td>
                                <td class="align-middle">@if(isset($payment->installment5_remaining_amount)){{ number_format($payment->installment5_remaining_amount,2,",",".") }}@if($payment->currency_unit=="Türk Lirası") TL @elseif($payment->currency_unit=="Dolar") $ @endif <br> {{ $payment->installment_date_format(5) }}@endif</td>
